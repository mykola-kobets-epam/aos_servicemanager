...
func (instance *Logging) GetInstanceLog(request cloudprotocol.RequestLog) error {
	...
	if len(logRequest.instanceIDs) == 0 {
		log.WithField("logID", request.LogID).Error("No instance ids for log request")
...
func (instance *Logging) getInstanceCrashLog(request getLogRequest) (err error) {
	...
	if crashTime == 0 {
		log.WithFields(log.Fields{"logID": request.logID, "instanceIDs": request.instanceIDs}).Error("No instance crash found")
		instance.sendEmptyResponse(request.logID)
